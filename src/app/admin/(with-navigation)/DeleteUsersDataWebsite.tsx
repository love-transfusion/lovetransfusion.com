'use client'
import { supa_delete_users_data_website } from '@/app/_actions/users_data_website/actions'
...
const DeleteUsersDataWebsite = ({ user_id }: { user_id: string }) => {
  ...
  const handleDelete = async () => {
    setisLoading(true)
    const { error } = await supa_delete_users_data_website(user_id)
    if (error) settoast({ clDescription: error, clStatus: 'error' })
    clTogglePopup()
  }
  return (
    <>
...
export default DeleteUsersDataWebsite
